...
@section('content')
  ...
  <div class="text-sm text-gray-600 mb-4">
    De: <b>{{ $ticket->user->name }}</b> ({{ $ticket->user->email }}) |
  ...
  @if($ticket->admin_reply)
    <div class="p-4 border rounded mb-4">
      ...
      <div class="text-sm text-gray-600 mt-2">
        Por: {{ optional($ticket->answeredBy)->name }} | {{ optional($ticket->answered_at)->format('Y-m-d H:i') }}
      </div>
    </div>
  @endif

  <form method="POST" action="{{ route('admin.tickets.reply', $ticket) }}" class="space-y-3 mb-4">
    @csrf
