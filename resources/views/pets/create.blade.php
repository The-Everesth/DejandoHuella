<x-app-layout>
	<x-page-title title="Registrar mascota" />
	<div class="max-w-lg mx-auto">
		...
		@endif
		<form method="POST" action="{{ route('my.pets.store') }}" class="space-y-4">
			@csrf
			<div>
				<label class="block font-bold">Nombre</label>
				<input name="name" class="input w-full" required maxlength="100" value="{{ old('name') }}">
			</div>
			<div>
				<label class="block font-bold">Especie</label>
				<select name="species" class="input w-full" required>
					<option value="">Selecciona...</option>
					<option value="perro" @selected(old('species')=='perro')>Perro</option>
					<option value="gato" @selected(old('species')=='gato')>Gato</option>
					<option value="otro" @selected(old('species')=='otro')>Otro</option>
				</select>
			</div>
			<div>
			...
			<div>
				...
				<input name="breed" class="input w-full" maxlength="50" value="{{ old('breed') }}">
			</div>
			<div>
				<label class="block font-bold">Edad (años)</label>
				<input name="ageYears" type="number" min="0" max="50" class="input w-full" value="{{ old('ageYears') }}">
			</div>
			<div>
				<label class="block font-bold">Notas</label>
				<textarea name="notes" class="input w-full" maxlength="255">{{ old('notes') }}</textarea>
			</div>
